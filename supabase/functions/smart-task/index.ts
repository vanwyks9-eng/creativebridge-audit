/**
 * CreativeBridge Audit — Supabase Edge Function v4.0
 * Slug: smart-task
...
function buildSummaryEmail(r: Record<string, any>, reportUrl: string): string {
  ...
  const topWins = tier === "Pro"
    ? (r.top5QuickWins||[]).slice(0,3).map((w: string) => `<div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid #E8E7F5;"><span style="color:#1D7A45;font-weight:700;flex-shrink:0;font-size:16px;">✓</span><span style="font-size:13px;color:#3D3C55;line-height:1.5;">${w}</span></div>`).join("")
...
<tr><td style="background:#fff;padding:32px 40px 40px;">
  <div style="text-align:center;background:#F4F3FF;border-radius:12px;padding:28px;">
    ...
    <div style="font-size:13px;color:#6B6A80;margin-bottom:20px;">All ${tier === "Pro" ? "10 category scores, detailed observations, phased roadmap" : "5 category scores, strengths, issues, and quick wins"}. Download as PDF to share with your team.</div>
...
async function processAudit(form: Record<string, string>, submissionId: string) {
  ...
  try {
    ...
    // Save full report and get report URL
    const reportUrl = `${REPORT_BASE}?id=${submissionId}`;

    // Build and send slim summary email
    const html = buildSummaryEmail(report as Record<string, any>, reportUrl);
    const subject = `Your ${(report as any).detectedCategory || ""} UX Audit — ${form.companyName}`;
    await sendEmail(form.email, subject, html);

    // Update submission with full report
